...
## `dispute_deadline_tracker.py` — ⚠ устарел

Первый инструмент проекта; полностью заменён `order_ledger.py` + `remind.py`.
Работает, но не развивается (T-065). Для новых заказов используй ledger.
...
Настройка Telegram-уведомлений под ключ: `docs/REMINDERS_SETUP.md`.
Токены бота — только в vault (env TG_TOKEN/TG_CHAT подхватываются автоматически).
